...
public class UtilScalarWave {
	...
	public static double gaussian_pulse(double n, double S) {
		double arg = (-1) * Math.pow((n * S - 40), 2) / 200;
		return Math.exp(arg);
	}

	public static double source1(int n) {
		double arg = (-1) * Math.pow((n - 60), 2) / 200;
		return Math.exp(arg);
	}

	public static double source2(int n, double S) {
	...
	public static double[][] FDTD(double[] S, int I, int N, int gauss_method) {
		...
		u[0][0] = (gauss_method == 0) ? gaussian_pulse(0, S[0]) : (gauss_method == 1) ? source1(0) : source2(0, S[0]);
		for (int i = 2; i < N; i++) {
			u[i][0] = (gauss_method == 0) ? gaussian_pulse(i, S[0]) : (gauss_method == 1) ? source1(i) : source2(i, S[0]);
			for (int j = 1; j < (I - 1); j++) {
